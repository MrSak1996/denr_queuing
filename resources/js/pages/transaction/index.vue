...
<template>
    ...
    <modal_client_type v-if="openClientType" :open="openClientType" @close="openClientType=false"></modal_client_type>

    <Head title="Client" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="h-[500px] w-full flex-1 overflow-hidden rounded border bg-cover bg-center p-4 text-center text-white shadow transition-all duration-1000 ease-in-out"
            :style="{ backgroundImage: `url(${denr_wall})` }">
            <div class="mt-64 flex flex-col items-center gap-10 rounded-lg p-6 md:p-24">
                <div class="flex w-full justify-center">
                    <Button
                        class="h-[100px] w-full border !bg-gradient-to-r from-blue-950 to-green-700 text-white md:w-1/2 text-center p-2"
                        @click="openClientType=true">
                        <div class="flex flex-col items-center justify-center leading-tight">
                            <span class="text-[40px] font-extrabold md:text-[65px]">PACDO</span>
                            <span class="text-[30px] md:text-[14px] text-center">
                                (LICENSES, PERMITTING, CERTIFICATION AND OTHER CONCERNS)
                            </span>
                        </div>
                    </Button>
                </div>
                ...
                <div class="flex w-full justify-center">
                    <Button
                        class="h-[100px] w-full border !bg-gradient-to-r from-blue-950 to-green-700 text-white md:w-1/2"
                        @click="openClientType=true">
